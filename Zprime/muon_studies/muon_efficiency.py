...
"""
muon_efficiency.py
This script creates plots used to generate muon-pT-wise efficiency for old and new muon selection
...
def main():
    ...
    loop = pyframe.core.EventLoop(name='muon_efficiency_test', version=version, quiet=False)
    ...
    #old muon selection
    muon_old_selector = pyframe.muon.MuonSelector(
        min_pt = 4.0*GeV,
        max_abs_eta = 2.5,
        flags = ['loose', 'isCombinedMuon', 'truth_matched'],
        req_expected_blayer_hits = True,
        min_expected_pixel_hits = 1,
        min_expected_sct_hits = 5,
        max_si_holes = 2,
        req_trt_quality = True,
        )
    loop += pyframe.selectors.SelectorAlg('OldMuonSelector',
                                          selector = muon_old_selector,
                                          key_in='staco_muons',
                                          key_out='old_muons',
                                          )
    muon_old_iso_selector = pyframe.muon.MuonSelector(
        max_scores = [('ptcone40rel', 0.06),
                      ('corr_etcone20rel', 0.06),
                      ],
        )
    loop += pyframe.selectors.SelectorAlg('OldMuonOldIsoSelector',
                                          selector = muon_old_iso_selector,
                                          key_in='old_muons',
                                          key_out='old_muons_old_iso',
                                          )
    ...
    loop += pyframe.selectors.SelectorAlg('NewMuonOldIsoSelector',
                                          selector = muon_old_iso_selector,
                                          key_in='new_muons',
                                          key_out='new_muons_old_iso',
                                          )
    ...
    muon_new_highpt_selector = pyframe.muon.MuonSelector(
        max_d0 = 0.2,
        max_z0 = 1.0,
    ...
    loop += pyframe.selectors.SelectorAlg('NewMuonHighPtSelector',
                                        selector = muon_new_highpt_selector,
                                        key_in='new_muons',
