...
This script runs the procedure for making the plots plots used for muon resolution studies (no 3s or IP)
"""
...
def main():
    ...
    loop = pyframe.core.EventLoop(name='muon_resolution_5_3S2S_test', version=version, quiet=False)
    ...
    muon_new_veto_selector_2s = pyframe.muon.MuonSelector(
        highpt_veto_MS_chambers = True,
        veto_endcap = True,
        )
    ...
    muon_new_phi_selector_2s = pyframe.muon.MuonSelector(
        highpt_phi_hits2 = True,
        )
    ...
    loop += pyframe.selectors.SelectorAlg('NewMuon2sSelector',
                                        ...
                                        key_in='new_muons',
                                        key_out='new_IP_2S_muons',
                                          )
    loop += pyframe.algs.ListMerger( keys_in = ['new_IP_3S_muons', 'new_IP_2S_muons'],
                                     key_out = 'new_IP_2S3S_muons',
                                     )
    loop += pyframe.selectors.SelectorAlg('NewMuonVetoSelector',
                                        selector = muon_new_veto_selector,
                                        key_in='new_IP_3S_muons',
                                        key_out='new_IP_3S_veto_muons',
                                        )
    loop += pyframe.selectors.SelectorAlg('NewMuonVetoSelector2s',
                                        selector = muon_new_veto_selector_2s,
                                        key_in='new_IP_2S_muons',
                                        key_out='new_IP_2S_veto_muons',
                                        )
    loop += pyframe.algs.ListMerger( keys_in = ['new_IP_3S_veto_muons', 'new_IP_2S_veto_muons'],
                                     key_out = 'new_IP_2S3S_veto_muons',
                                     )
    loop += pyframe.selectors.SelectorAlg('NewMuonPhiSelector',
                                        selector = muon_new_phi_selector,
                                        key_in='new_IP_3S_veto_muons',
                                        key_out='new_IP_3S_veto_phi_muons',
                                        )
    loop += pyframe.selectors.SelectorAlg('NewMuonPhiSelector2s',
                                        selector = muon_new_phi_selector_2s,
                                        key_in='new_IP_2S_veto_muons',
                                        key_out='new_IP_2S_veto_phi_muons',
                                        )
    loop += pyframe.algs.ListMerger( keys_in = ['new_IP_3S_veto_phi_muons', 'new_IP_2S_veto_phi_muons'],
                                     key_out = 'new_IP_2S3S_veto_phi_muons',
                                     )
    loop += pyframe.selectors.SelectorAlg('NewMuonIDMSSelector',
                                        selector = muon_new_IDMS_selector,
                                        key_in='new_IP_3S_veto_phi_muons',
                                        key_out='new_highpt_3S_muons',
                                        )
    loop += pyframe.selectors.SelectorAlg('NewMuonIDMSSelector2s',
                                        selector = muon_new_IDMS_selector_2s,
                                        key_in='new_IP_2S_veto_phi_muons',
                                        key_out='new_highpt_2S_muons',
                                        )
    loop += pyframe.algs.ListMerger( keys_in = ['new_highpt_3S_muons', 'new_highpt_2S_muons'],
    ...
    #    )
    loop += pyframe.algs.LooperAlg(
        key = 'new_IP_2S3S_muons',
        func = TS.muon_resolution_plots.plot_mu_pt,
        prefix = 'new_mu_IP_2S3S_',
        dir='',
        )
    loop += pyframe.algs.LooperAlg(
        key = 'new_IP_2S3S_veto_muons',
        func = TS.muon_resolution_plots.plot_mu_pt,
        prefix = 'new_mu_IP_2S3S_veto_',
        dir='',
        )
    loop += pyframe.algs.LooperAlg(
        key = 'new_IP_2S3S_veto_phi_muons',
        func = TS.muon_resolution_plots.plot_mu_pt,
        prefix = 'new_mu_IP_2S3S_veto_phi_',
        dir='',
        )
    loop += pyframe.algs.LooperAlg(
