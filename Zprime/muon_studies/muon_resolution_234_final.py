...
def main():
    ...
    # load file
    infile = 'muon_resolution_%i%s_test.DY.hist.root' % (ops.selection, zoomed)
    ...
    hists_2d.append((g_new_mu_IP_3S_veto_pT_resolution, 'new_mu_IP_3S_veto_pT_resolution'))
    ...
    hists_2d.append((g_new_mu_IP_3S_veto_phi_pT_resolution, 'new_mu_IP_3S_veto_phi_pT_resolution'))
    ...
    if ops.selection > 3:
        ...
        hists_2d.append((g_new_mu_IP_3S_pT_resolution, 'new_mu_IP_3S_pT_resolution'))
    ...
    file_out = ROOT.TFile('muon_resolution_prjection_%i%s.root' % (ops.selection, zoomed) , 'RECREATE')
    ...
    for x in range(6):
        keylist[x] = x
    namelist = ['new_mu_pT_resolution','new_mu_iso_pT_resolution','new_mu_IP_3S_veto_pT_resolution','new_mu_IP_3S_veto_phi_pT_resolution']
    ...
    if ops.selection > 3:
        ...
        if ops.selection < 5:
            slot += 1
        namelist.insert(slot , 'new_mu_IP_3S_pT_resolution')
        colorlist.insert(slot , 30)
        legendlist.insert(slot , '+3 station (no iso)')
    ...
    for bin in range(numbins):
        ...
        leg.Draw()
        c1.Update()
        c1.SaveAs('muon_resolution_%i%s_pT_%i%s.pdf' % (ops.selection, zoomed, bin*100/mag, log))
        c1.Clear()
    ...
    for graph, name, heading in mgs:
        ...
        c1.Update()
        c1.SaveAs('muon_resolution_%i%s_%s_plot%s.pdf' % (ops.selection, zoomed, name, log))
        c1.Clear()
