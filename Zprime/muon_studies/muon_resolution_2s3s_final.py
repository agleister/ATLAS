...
def main():
    ...
    # load file
    infile = 'muon_resolution_%i%s_3S2S_test.DY.hist.root' % (ops.selection, zoomed)
    ...
    hists_2d.append((g_new_mu_iso_pT_resolution, 'new_mu_iso_pT_resolution'))
    g_new_mu_IP_2S3S_veto_pT_resolution = ROOT.TH2F()
    file_init.GetObject('g_new_mu_IP_2S3S_veto_pT_resolution', g_new_mu_IP_2S3S_veto_pT_resolution)
    hists_2d.append((g_new_mu_IP_2S3S_veto_pT_resolution, 'new_mu_IP_2S3S_veto_pT_resolution'))
    g_new_mu_IP_2S3S_veto_phi_pT_resolution = ROOT.TH2F()
    file_init.GetObject('g_new_mu_IP_2S3S_veto_phi_pT_resolution', g_new_mu_IP_2S3S_veto_phi_pT_resolution)
    hists_2d.append((g_new_mu_IP_2S3S_veto_phi_pT_resolution, 'new_mu_IP_2S3S_veto_phi_pT_resolution'))
    g_new_mu_highpt_pT_resolution = ROOT.TH2F()
    file_init.GetObject('g_new_mu_highpt_pT_resolution', g_new_mu_highpt_pT_resolution)
    hists_2d.append((g_new_mu_highpt_pT_resolution, 'new_mu_highpt_pT_resolution'))
    ...
    hists_2d.append((g_new_mu_highpt_iso_pT_resolution, 'new_mu_highpt_iso_pT_resolution'))

    if ops.selection > 2 and ops.selection < 5:
        g_new_mu_IP_pT_resolution = ROOT.TH2F()
        file_init.GetObject('g_new_mu_IP_pT_resolution', g_new_mu_IP_pT_resolution)
        hists_2d.append((g_new_mu_IP_pT_resolution,'new_mu_IP_pT_resolution'))

    if ops.selection > 3:
        g_new_mu_IP_2S3S_pT_resolution = ROOT.TH2F()
        file_init.GetObject('g_new_mu_IP_2S3S_pT_resolution', g_new_mu_IP_2S3S_pT_resolution)
        hists_2d.append((g_new_mu_IP_2S3S_pT_resolution, 'new_mu_IP_2S3S_pT_resolution'))
    ...
    file_out = ROOT.TFile('muon_resolution_prjection_%i%s_2S3S.root' % (ops.selection, zoomed) , 'RECREATE')
    ...
    for x in range(6):
        keylist[x] = x
    namelist = ['new_mu_pT_resolution','new_mu_iso_pT_resolution','new_mu_IP_2S3S_veto_pT_resolution','new_mu_IP_2S3S_veto_phi_pT_resolution']
    namelist.extend(['new_mu_highpt_pT_resolution','new_mu_highpt_iso_pT_resolution' ])
    colorlist = [1, 6, 3, 41, 28, 2]
    legendlist = ['2012 muon selection', '+isolaton', '+veto (no iso)']
    legendlist.extend(['+phi (no iso)', 'full highpT (no iso)', 'full highpT+iso'])

    if ops.selection > 2 and ops.selection < 5:
        keylist.append(6)
        namelist.insert(2, 'new_mu_IP_pT_resolution')
        colorlist.insert(2, 4)
        legendlist.insert(2, '+d0,z0 (no iso)')

    if ops.selection > 3:
        keylist.append(len(keylist[:]))
        slot = 2
        if ops.selection < 5:
            slot += 1
        namelist.insert(slot , 'new_mu_IP_2S3S_pT_resolution')
        colorlist.insert(slot , 30)
        legendlist.insert(slot , '+3 or 2 station (no iso)')
    ...
    for bin in range(numbins):
        ...
        leg.Draw()
        c1.Update()
        c1.SaveAs('muon_resolution_%i%s_2S3S_pT_%i%s.pdf' % (ops.selection, zoomed, bin*100/mag, log))
        c1.Clear()
    ...
    for graph, name, heading in mgs:
        ...
        c1.Update()
        c1.SaveAs('muon_resolution_%i%s_%s_plot_2S3S%s.pdf' % (ops.selection, zoomed, name, log))
        c1.Clear()
